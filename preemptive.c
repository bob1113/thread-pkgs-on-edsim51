...
// __data __at (0x38) char thread_full; 
...
ThreadID ThreadCreate(FunctionPtr fp) {
   // SemaphoreWait(thread_empty, label);
   EA = 0;
   /*
    * REVIEW:
...
void ThreadExit(void) {
   // SemaphoreWait(thread_full, label);
   EA = 0;
   /* REVIEW:
...
void myTimer0Handler(void) {
   ...
   do {         
      if (threadID_current < 3) 
         ...
         threadID_current = 0;
      if ((1 << threadID_current) & thread_bitmap) break;
   } while (1);
